...
public class SensorEventPublisherImpl implements SensorEventPublisher {
	...
	LOG.info("Attempting to get topic: {} using Broker URL: {} and User ID: {}...", getTopicName(), getBrokerUrl(),
		getUserId());
	...
	topic = client.getTopic(getTopicName());
	LOG.info("Retrieved topic: {}.", getTopicName());
    }

    @Override
    public void publish(final SensorEvent sensorEvent) {
	final String sensorEventJson = convertSensorEventToJson(sensorEvent);
	LOG.info("Publishing on topic: {} Sensor Event: {} using JSON: {}...", getTopicName(), sensorEvent,
		sensorEventJson);
	try {

	    topic.publish(new MqttMessage(sensorEventJson.getBytes()));
	} catch (final MqttException e) {
	    LOG.error("Failed to publish on topic: {} Sensor Event: {} using JSON: {}.", getTopicName(), sensorEvent,
		    sensorEventJson);
	    return;
	}

	LOG.info("Successfully published on topic: {} Sensor Event: {} using JSON: {}.", getTopicName(), sensorEvent,
		sensorEventJson);
